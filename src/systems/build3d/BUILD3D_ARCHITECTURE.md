...
- **Dual-Path Change Detection:** The Inspector uses *two distinct mechanisms* to modify entities, requiring both a `MutationObserver` (catches direct DOM attribute writes like `element.setAttribute('geometry', '...')`) and an A-Frame `componentchanged` event listener (catches API-level component modifications). The debounce buffer naturally deduplicates overlapping events from both sources. Note: `componentinitialized` is intentionally **not** used — it fires asynchronously during scene load for components like `gltf-model` and `attribution`, causing spurious republishing of every persisted object. The MutationObserver already catches new component additions at the DOM level.
...
- **Component Modifications (`componentchanged`):** Leverages A-Frame's native event emitter to catch component property changes made through the A-Frame API.
- **Component Removal (`componentremoved`):** Publishes `null` for removed components, signaling deletion on the wire. Inbound side (`create-update.js`) handles `null` by calling `entityEl.removeAttribute()`.
