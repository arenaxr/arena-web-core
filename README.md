...
```
#### Particles
Particles are based on https://github.com/harlyq/aframe-spe-particles-component 
For now not directly supported, but rather by passing JSON inside the `data{}` element. The syntax for parameter names has been updated so instead of a name like this that is `space-separated` instead they are `spaceSeparated` (camel case). Three examples here have been created by starting from the examples in view-source:https://harlyq.github.io/aframe-spe-particles-component/ then reformulating the syntax into JSON:
```
