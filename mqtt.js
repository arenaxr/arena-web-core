...
ARENA.mqttClient = new Paho.Client(globals.mqttParam,
    'webClient-' + globals.timeID);
...
const loadArena = (urlToLoad, position, rotation) => {
    ...
    xhr.onload = () => {
        if (xhr.status !== 200) {
            alert(
                `Error loading initial scene data: ${xhr.status}: ${xhr.statusText}`);
        } else {
            ...
            for (let i = 0; i < l; i++) {
                ...
                if (obj.attributes.parent) {
                    ...
                    if (rotation) {
                        const r = new THREE.Quaternion(msg.data.rotation.x,
                            msg.data.rotation.y, msg.data.rotation.z,
                            msg.data.rotation.w);
                        const q = new THREE.Quaternion(rotation.x, rotation.y,
            ...
            for (let i = 0; i < l2; i++) {
                ...
                console.log(
                    'adding deferred object ' + obj.object_id + ' to parent ' +
                    obj.attributes.parent);
                onMessageArrived(undefined, msg);
            }
        }
    };
        ...
        if (xhr.status !== 200) {
            ...
            if (payload) {
                ...
                for (const [attribute, value] of Object.entries(envPresets)) {
                    globals.sceneObjects.env.setAttribute('environment',
                        attribute, value);
                }
                document.getElementById('sceneRoot').
                    appendChild(globals.sceneObjects.env);
            } else {
                // set defaults
                globals.sceneObjects.env.setAttribute('environment', 'preset',
                    'starry');
                ...
                globals.sceneObjects.env.setAttribute('environment', 'grid',
                    'none');
...
window.addEventListener('onauth', function(e) {
    ...
    ARENA.mqttClient.connect({
        ...
        onFailure: function(res) {
            console.error(
                `MQTT scene connection failed, ${res.errorCode}, ${res.errorMessage}`);
        },
        reconnect: true,
        willMessage: lwt,
    ...
    ARENA.RuntimeManager.init({
        ...
        name: 'rt-' + Math.round(Math.random() * 10000) + '-' +
            globals.username,
        dbg: false,
        mqtt_username: globals.username,
        mqtt_token: globals.mqttToken,
...
function onConnected(reconnect, uri) {
    ...
    if (globals.weatherParam !== 'none') {
        if (sceneObjects.weather) {
            sceneObjects.weather.setAttribute('particle-system', 'preset',
                globals.weatherParam);
            sceneObjects.weather.setAttribute('particle-system', 'enabled',
                'true');
        }
    ...
    sceneObjects.myCamera.addEventListener('poseChanged', (e) => {
        ...
        const msg = {
            ...
            hasAvatar: (ARENA.FaceTracker !== undefined) &&
                ARENA.FaceTracker.hasAvatar(),
    ...
    try {
        ...
        if (message) {
            if (message.payloadString) {
                ...
                console.error('onMessageArrived Error!', err,
                    new TextDecoder('utf-8').decode(message.payloadBytes),
...
function _onMessageArrived(message, jsonMessage) {
    ...
    case 'clientEvent': {
        ...
        if (theMessage.data.position) {
            myPoint = new THREE.Vector3(
                parseFloat(theMessage.data.position.x),
                parseFloat(theMessage.data.position.y),
                parseFloat(theMessage.data.position.z));
        } else {
            console.error('Error: theMessage.data.position not defined',
                theMessage);
        }
    ...
    case 'create': {
        ...
        if (name in sceneObjects) {
            ...
            if (type === 'viveLeft' || type === 'viveRight') {
                ...
                if (type === 'viveLeft') {
                    entityEl.setAttribute('gltf-model',
                        'url(models/valve_index_left.gltf)');
                } else {
                    entityEl.setAttribute('gltf-model',
                        'url(models/valve_index_right.gltf)');
                }
                ...
                headModelEl.setAttribute('gltf-model',
                    'url(models/Head.gltf)'); // actually a face mesh
                ...
                const personName = decodeName.normalize('NFD').
                    replace(/[\u0300-\u036f]/g, '');
                ...
                if (theMessage.data.parent) {
                    ...
                    if (parentEl) {
                        ...
                        console.log(
                            'orphaned; parent ' + name + ' cannot find ' +
                                theMessage.data.parent + ' yet');
                    }
                } else {
                    sceneObjects.scene.appendChild(entityEl);
                    // Add it to our dictionary of scene objects
            ...
            if (theMessage.hasOwnProperty('displayName')) {
                ...
                for (const child of entityEl.children) {
                    if (child.getAttribute('id').
                        includes('headtext_')) {
                        // TODO(mwfarb): support full unicode in a-frame text, until then, normalize headtext
                        const name = theMessage.displayName.normalize(
                            'NFD').replace(/[\u0300-\u036f]/g, '');
        ...
        case 'videoconf':
            ...
            if (theMessage.hasOwnProperty('jitsiId')) {
                ...
                if (theMessage.hasVideo &&
                            ARENA.JitsiAPI.activeSpeakerChanged()) {
        ...
        case 'camera':
            ...
            if (theMessage.hasOwnProperty('jitsiId')) {
                if (theMessage.jitsiId === '' ||
                            (ARENA.JitsiAPI && !ARENA.JitsiAPI.ready())) {
                ...
                if (theMessage.hasVideo) {
                    if (document.getElementById(videoID) &&
                                !(entityEl.getAttribute('videoCubeDrawn') ==
                                    'true')) {
                    ...
                    const vidCube = document.getElementById(
                        videoID + 'cube');
                    if (entityEl.contains(vidCube)) {
                        entityEl.removeChild(vidCube);
                    }
                    const vidCubeDark = document.getElementById(
                        videoID + 'cubeDark');
                ...
                if (theMessage.hasAudio) {
                    ...
                    const audioTrack = ARENA.JitsiAPI.getAudioTrack(
                        theMessage.jitsiId);
                    ...
                    if (entityEl.audioTrack !== oldAudioTrack) {
                        ...
                        if (!entityEl.positionalAudio) {
                            const audioSource = new THREE.PositionalAudio(
                                listener);
                            audioSource.setMediaStreamSource(
                                audioStream);
                            entityEl.positionalAudio = audioSource;
                            entityEl.object3D.add(audioSource);
                        } else {
                            entityEl.positionalAudio.setMediaStreamSource(
                                audioStream);
                        }
                        if (globals.refDist) {
                            entityEl.positionalAudio.setRefDistance(
                                globals.refDist);
                        } // L-R panning
                        if (globals.rolloffFact) {
                            entityEl.positionalAudio.setRolloffFactor(
                                globals.rolloffFact);
                        }
                        if (globals.distModel) {
                            entityEl.positionalAudio.setDistanceModel(
                                globals.distModel);
                        }
                        if (globals.maxDist) {
                            entityEl.positionalAudio.setMaxDistance(
                                globals.maxDist);
                        }
                        if (globals.volume) {
                            entityEl.positionalAudio.setVolume(
                                globals.volume);
                        }
                            ...
                            setTimeout(function() {
                                if (audioCtx.state === 'running') {
                                    ...
                                    document.body.removeEventListener(
                                        'touchmove', resume, false);
                                    document.body.removeEventListener(
                                        'mousemove', resume, false);
                                }
                            }, 0);
                        };
        ...
        case 'image': // use special 'url' data slot for bitmap URL (like gltf-models do)
            ...
            entityEl.setAttribute('material', 'src',
                theMessage.data.url);
        ...
        case 'gltf-model':
            ...
            function updateProgress(failed, evt) {
                const gltfProgressEl = document.getElementById(
                    'gltf-loading');
                ...
                for (const [src, progress] of Object.entries(progMsgs)) {
                    if (progress === 'failed') {
                        ...
                        const shortName = src.length < 20 ?
                            src :
                            '…' + src.substring(src.length - 20);
                        innerHTML += shortName + '<br/>' +
                                parseFloat(progress.toFixed(1)) + '%' + '<br/>';
                    }
                }
        ...
        case 'text':
            ...
            if (typeof theText === 'string' || theText instanceof
                        String) {
                entityEl.setAttribute('text', 'value',
                    theMessage.data.text);
    ...
    case 'update': {
        ...
        case 'rig': {
            if (name === globals.camName) { // our camera Rig
                ...
                sceneObjects.cameraSpinner.object3D.quaternion.set(xrot,
                    yrot, zrot, wrot);
            ...
            if (entityEl) {
                for (const [attribute, value] of Object.entries(
                    ...
                    if (attribute === 'rotation') {
                        entityEl.object3D.quaternion.set(value.x,
                            value.y, value.z, value.w);
                    } else if (attribute === 'position') {
                        entityEl.object3D.position.set(value.x, value.y,
                            value.z);
                    } else {
                        if (value === null) {
                ...
                console.log('Warning: ' + theMessage.object_id +
                            ' not in sceneObjects');
            }
            break;
        }
        default:
            console.log('EMPTY MESSAGE?', message.destinationName,
                message.payloadString);
            break;
        } // switch (theMessage.type)
    } // case "update":
