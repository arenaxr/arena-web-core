...
window.addEventListener('onauth', async function (e) {
    ...
    */
    ...
    if (sceneinput.disabled === false) await PersistObjects.populateObjectList(`${namespaceinput.value}/${sceneinput.value}`, objFilter.value, objTypeFilter, focusObjectId);
    localStorage.setItem("scene", sceneinput.value );
    reload();
    updateLink();
    updateUrl();
