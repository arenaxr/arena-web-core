...
export default class ARENAChat {
    ...
    constructor(st) {
        ...
        this.settings = {
            ...
            username:
                st.username !== undefined ? st.username : "chat-dft-username",
            ...
            keepalive_interval_ms:
                st.keepalive_interval_ms !== undefined
                    ? st.keepalive_interval_ms
                    : 30000,
            mqtt_host:
            ...
            supportDevFolders:
                st.supportDevFolders !== undefined
                    ? st.supportDevFolders
                    : false,
        };
        ...
        setInterval(
            this.userCleanup.bind(this),
            this.settings.keepalive_interval_ms * 3
        );
     ...
     /*
	   ...
	   */
        ...
        label = document.createElement("span");
        label.innerHTML =
            "<br/><br/>&nbspLandmarks (buttons allow to find landmarks):";
        label.style.fontSize = "small";
        ...
        this.displayAlert(
            "Not sending audio or video. Use icons on the right to start.",
            5000
        );
        ...
        this.msgBtn.onclick = function () {
            if (_this.msgTxt.value.length > 0)
                _this.sendMsg(_this.msgTxt.value);
            _this.msgTxt.value = "";
        };
        ...
        this.msgTxt.addEventListener("keyup", function (event) {
            ...
            if (event.keyCode === 13) {
                if (_this.msgTxt.value.length > 1)
                    _this.sendMsg(_this.msgTxt.value);
                _this.msgTxt.value = "";
            }
        });
        ...
        this.silenceAllBtn.onclick = function () {
            if (!_this.isUserAuthenticated(_this.settings.cameraid)) {
                _this.displayAlert(
                    "Anonymous users may not mute others.",
                    3000
                );
                return;
            }
        ...
        ARENA.events.on(
            ARENAEventEmitter.events.JITSI_CONNECT,
            this.jitsiConnectCallback
        );
        ARENA.events.on(
            ARENAEventEmitter.events.USER_JOINED,
            this.userJoinCallback
        );
    }
    ...
    jitsiConnectCallback = (e) => {
        ...
        args.pl.forEach((user) => {
            ...
            // check if jitsi knows about someone we don'r; add to user list
            if (!this.liveUsers[user.cid]) {
    ...
    sendMsg(msgTxt) {
        ...
        let dstTopic =
            this.toSel.value == "scene" || this.toSel.value == "all"
                ? this.settings.publishPublicTopic
                : this.settings.publishPrivateTopic.replace(
                      "{to_cid}",
                      this.toSel.value
                  );
    ...
    onMessageArrived(mqttMsg) {
        ...
        if (msg.from_cid == undefined) return;
        if (msg.to_cid == undefined) return;
        if (msg.from_cid == this.settings.cameraid) return;
        ...
        if (
            this.liveUsers[msg.from_cid] == undefined &&
            msg.from_un !== undefined &&
            msg.from_scene !== undefined
        ) {
            this.liveUsers[msg.from_cid] = {
                un: msg.from_un,
            ...
            this.liveUsers[msg.from_cid].un = msg.from_un;
            this.liveUsers[msg.from_cid].scene = msg.from_scene;
            this.liveUsers[msg.from_cid].cid = msg.cameraid;
            this.liveUsers[msg.from_cid].ts = new Date().getTime();
            if (msg.text) {
                if (msg.text == "left") {
                    delete this.liveUsers[msg.from_cid];
        ...
        if (msg.type == "chat-ctrl") {
            if (msg.text == "sound:off") {
                ...
                if (
                    abtn.style.backgroundImage.includes("audio-on.png") == true
                ) {
                    abtn.click();
                    this.displayAlert(
                        "Sound muted. Requested by " + msg.from_un + ".",
                        3000
                    );
                }
            }
        ...
        if (
            msg.to_cid !== this.settings.cameraid &&
            msg.to_cid !== "all" &&
            msg.to_cid !== "scene"
        )
            return;
        ...
        this.unreadMsgs++;
        this.chatDot.innerHTML =
            this.unreadMsgs < 100 ? this.unreadMsgs : "...";
    ...
    populateUserList(newUser = undefined) {
        ...
        Object.keys(this.liveUsers).forEach(function (key) {
            ...
            userList.push({
                uid: key,
                sort_key:
                    _this.liveUsers[key].scene == _this.settings.scene
                        ? "aaa"
                        : "zzz",
        ...
        userList.sort((a, b) =>
            ("" + a.sort_key + a.scene + a.un).localeCompare(
        ...
        userList.forEach((user) => {
            ...
            op.innerHTML =
                "to: " +
                decodeURI(user.un) +
                (user.scene != _this.settings.scene
                    ? " (" + user.scene + ")"
                    : "");
    ...
    ctrlMsg(to, text, tryconnect = false) {
        ...
        if (to === "all" || to === "scene") {
            ...
            // replace '{to_cid}' for the 'to' value
            dstTopic = this.settings.publishPrivateTopic.replace(
                "{to_cid}",
                to
            );
        }
    ...
    userCleanup() {
        ...
        Object.keys(_this.liveUsers).forEach(function (key) {
            if (
                now - _this.liveUsers[key].ts >
                _this.settings.keepalive_interval_ms
            ) {
