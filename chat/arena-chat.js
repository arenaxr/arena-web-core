...
export default class ARENAChat {
  ...
  onMessageArrived(mqttMsg) {
    ...
    if (
      this.liveUsers[msg.from_uid] == undefined &&
    ...
    if (
      msg.to_uid !== this.settings.userid &&
      msg.to_uid !== "all" &&
      msg.to_uid !== "scene"
    )
      return;
  ...
  populateUserList() {
    ...
    Object.keys(this.liveUsers).forEach(function (key) {
      userList.push({
        uid: key,
        sort_key:
          _this.liveUsers[key].scene == _this.settings.scene ? "aaa" : "zzz",
    ...
    userList.forEach((user) => {
      ...
      op.innerHTML =
        "to: " +
        decodeURI(user.un) +
        (user.scene != _this.settings.scene ? " (" + user.scene + ")" : "");
  ...
  async populateLandmarkList() {
    try {
      let data = await fetch(
        this.settings.persist_uri + this.settings.scene + "?type=landmarks"
      );
      if (!data) {
    ...
    this.landmarks.forEach((lm) => {
      let uli = document.createElement("li");
      uli.innerHTML =
        lm.label.length > 45 ? lm.label.substring(0, 45) + "..." : lm.label;
